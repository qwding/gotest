...
func main() {
	...
	//filemode, err := os.Stat("pathDir.go")
	...
	err = os.Chmod("pathDir.go", 7777)
	fmt.Println("chmod:", err)
	filemode, err = os.Stat("pathDir.go")
	fmt.Println("Filemode:", filemode.Mode())

	//access time modification time
	err = os.Chtimes("pathDir.go", time.Now(), time.Now())
	...
	err = os.Link("pathDir.go", "newmain.go")
	if err != nil {
		fmt.Println(err)
	}
	...
	err = os.Rename("pathDir.go", "main1.go")

	f1, _ := os.Stat("pathDir.go")
